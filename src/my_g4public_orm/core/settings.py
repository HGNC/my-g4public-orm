"""MySQL database configuration settings (db-common subclass).

This mirrors ``pg_g4public_orm/core/settings.py`` but carries **MySQL** defaults
...
class DatabaseSettings(db_common.DatabaseSettings):
    """Database connection settings for the MySQL ``g4public`` database.
    ...
    driver: str = Field(
        ...
        description="SQLAlchemy driver string (mysqlclient)",
    )
    host: str = Field(default="localhost", description="Database host")
    port: int = Field(default=3306, description="Database port (MySQL default)")
    database: str = Field(
        default="g4public",
