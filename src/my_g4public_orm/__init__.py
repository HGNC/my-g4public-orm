"""my-g4public-orm: SQLAlchemy 2.0 ORM for the MySQL ``g4public`` database.
...
``mysql+mysqldb`` (mysqlclient) driver. The ORM reads and writes **data
only** — it never creates, alters, or drops schema objects.

## Quick start

```python
from my_g4public_orm import DatabaseSettings, initialize_engine, get_readwrite_session
...
def _resolve_version() -> str:
    ...
    try:
        return version("my-g4public-orm")
...
from my_g4public_orm.core.session import (  # noqa: E402
    close_all_sessions,
    get_engine,
