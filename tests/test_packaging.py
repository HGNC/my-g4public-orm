...
Consumers only benefit from my-g4public-orm's inline type annotations if the
...
def test_py_typed_in_wheel(tmp_path: Path) -> None:
    ...
    wheels = sorted(tmp_path.glob("my_g4public_orm-*.whl"))
    assert wheels, f"no my_g4public_orm wheel produced in {tmp_path}"
    wheel = wheels[-1]

    with zipfile.ZipFile(wheel) as zf:
        namelist = zf.namelist()
    assert (
        "my_g4public_orm/py.typed" in namelist
